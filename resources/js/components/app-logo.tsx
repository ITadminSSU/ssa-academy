...
const AppLogo = ({ className, theme, variant = 'wordmark' }: { theme?: 'light' | 'dark'; className?: string; variant?: 'wordmark' | 'icon' | 'footer' }) => {
   ...
   const siteName = resolveSiteName(branding?.name);
   ...
   const logoDark = variant === 'icon' ? resolveLogo(branding?.logos?.icon, 'icon') : resolveLogo(branding?.logos?.dark, 'dark');
   const logoLight = variant === 'icon' ? resolveLogo(branding?.logos?.icon, 'icon') : resolveLogo(branding?.logos?.light, 'light');
   ...
   if (className?.includes('ssu-nav-logo')) {
      // theme="light" = black wordmark on light navbars (homepage, public pages).
      // theme="dark" = white wordmark on navy bars (course player, auth sidebar, hero).
      // Without a theme, default to black so OS dark mode cannot invert public nav.
      if (theme === 'dark') {
