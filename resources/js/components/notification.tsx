...
const Notification = () => {
   ...
   const openNotification = (id: string) => {
      ...
      setNotifications(snapshot.filter((item) => item.id !== id));
      router.visit(route('notifications.show', id), {
         onCancel: () => setNotifications(snapshot),
                     ...
                     notifications.map(({ id, data, created_at }) => {
                        ...
                        return (
                           <button
                              ...
                              className={cn(
                                 'hover:bg-accent/50 flex w-full flex-col gap-1 px-4 py-2 text-left transition-colors',
                              )}
                           >
