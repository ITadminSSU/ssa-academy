...
class ChatService
{
    ...
    private function adminInbox(): array
    {
        return ChatConversation::query()
            ...
            ->limit(200)
            ->get()
            ->map(fn (ChatConversation $conversation) => $this->formatConversationRow($conversation, auth()->user()))
            ->all();
    }
    ...
    public function canSend(User $user, ChatConversation $conversation): bool
    {
        ...
        return $this->access->canAccessCourseMessaging($user, $conversation->course);
    }
    ...
    {
        $conversation->load(['course.instructor.user', 'student:id,name,photo']);
        ...
        return [
            ...$this->formatConversationRow($conversation, $viewer),
            'messages' => $messages,
    ...
    public function sendMessage(
        ...
        if (blank($body) && ! $attachment) {
            abort(422, 'Please enter a message or attach an image.');
        }
    ...
    private function formatConversationRow(
        ...
        return [
            ...
            'preview' => $latest?->body ?: ($latest?->attachment ? 'Image attachment' : null),
